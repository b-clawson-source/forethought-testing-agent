...
import { autonomousTestRoutes } from './routes/autonomousTests';
...
app.use(cors());
app.use(compression());
app.use(express.json({ limit: '50mb' })); // Increased from 1mb for larger test data
app.use(morgan('dev'));
...
app.use('/api/autonomous-tests', autonomousTestRoutes);
