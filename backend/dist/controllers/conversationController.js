...
const forethoughtService_1 = require("../services/forethoughtService");
const openai_1 = __importDefault(require("openai"));
// Initialize services
const forethoughtService = new forethoughtService_1.ForethoughtService();
...
class ConversationController {
     ...
     */
    static async startFreeformConversation(req, res) {
        try {
            ...
            while (turnCount < maxTurns) {
                ...
                const shouldEnd = await ConversationController.shouldEndConversation(conversation.messages, persona);
                ...
                currentMessage = await ConversationController.generateCustomerResponse(conversation.messages, persona, forethoughtResponse.response);
                if (!currentMessage) {
     ...
     */
    static async generateCustomerResponse(messages, persona, lastAgentResponse) {
        if (!process.env.OPENAI_API_KEY) {
            ...
            const responses = fallbackResponses[persona] || fallbackResponses.polite;
        ...
        try {
            ...
            const prompt = `${personaDescriptions[persona] || personaDescriptions.polite}
...
Customer response:`;
     ...
     */
    static async shouldEndConversation(messages, persona) {
     ...
     */
    static async testForethought(req, res) {
        try {
