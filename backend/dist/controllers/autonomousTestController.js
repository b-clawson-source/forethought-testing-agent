...
const autonomousConversationService_1 = require("../services/autonomousConversationService");
class AutonomousTestController {
    constructor() {
        this.runningTests = new Map();
        /**
         * Start autonomous conversation testing
         */
        this.startAutonomousTest = async (req, res) => {
            try {
                const { initialPrompt, conversationCount = 10, maxTurns = 15, personas } = req.body;
                if (!initialPrompt) {
         ...
         */
        this.getTestStatus = async (req, res) => {
            try {
         ...
         */
        this.getConversationLogs = async (req, res) => {
            try {
                const { testId } = req.params;
                const testData = this.runningTests.get(testId);
         ...
         */
        this.getDetailedReport = async (req, res) => {
            try {
                const { testId } = req.params;
                const testData = this.runningTests.get(testId);
                if (!testData || testData instanceof Promise) {
    ...
    generateRecommendations(report) {
        ...
        const avgTurns = report.conversationLogs.reduce((sum, log) => sum + log.turns.length, 0) / report.conversationLogs.length;
        if (avgTurns > 10) {
...
//# sourceMappingURL=autonomousTestController.js.map