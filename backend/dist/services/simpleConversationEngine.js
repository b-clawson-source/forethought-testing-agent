...
exports.SimpleConversationEngine = void 0;
...
class SimpleConversationEngine {
    ...
    async startConversation(initialPrompt, persona) {
    ...
    async continueConversation(sessionId) {
        ...
        try {
            ...
            if (lastMessage.role === 'assistant') {
                ...
                const customerResponse = await this.generateCustomerMessage(conversation.persona, conversation.initialPrompt, conversation.messages);
    ...
    async generateCustomerMessage(persona, initialPrompt, messageHistory) {
    ...
    getPersonaPrompt(persona, initialPrompt, messageHistory) {
...
Keep responses under 150 words. Sound genuinely confused by technical jargon.`
    ...
    generateSupportResponse(customerMessage, messageHistory = []) {
        ...
        if (messageHistory.length > 2) {
            const customerSeemsUrgent = messageHistory.some(msg => msg.role === 'user' && (msg.content.toLowerCase().includes('urgent') ||
    ...
    shouldEndConversation(conversation) {
        ...
        const hasResolutionIndicator = lastMessage && resolvedKeywords.some(keyword => lastMessage.content.toLowerCase().includes(keyword));
...
exports.SimpleConversationEngine = SimpleConversationEngine;
