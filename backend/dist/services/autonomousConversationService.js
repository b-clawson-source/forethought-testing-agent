...
class AutonomousConversationService {
    constructor() {
     ...
     */
    async runConversationCycles(initialPrompt, config) {
        ...
        for (let i = 0; i < config.conversationCount; i++) {
            ...
            try {
                ...
                const conversationLog = await this.runSingleConversation(scenario, persona, config.maxTurns, `conv-${i + 1}`);
     ...
     */
    async runSingleConversation(scenario, persona, maxTurns, conversationId) {
        const log = {
            id: conversationId,
            startTime: new Date(),
            endTime: new Date(),
        ...
        while (turnCount < maxTurns) {
            ...
            currentMessage = await this.generateCustomerResponse(conversationContext, persona, forethoughtResponse.response);
     ...
     */
    async getForethoughtResponse(message, sessionId) {
        try {
     ...
     */
    async generateCustomerResponse(conversationContext, persona, agentResponse) {
        const prompt = `
...
Customer response:`;
     ...
     * Generate conversation scenarios based on initial prompt
     */
    async generateConversationScenarios(initialPrompt, count) {
        const prompt = `
...
Scenarios:`;
     ...
     */
    async shouldEndConversation(conversationContext, persona) {
        const prompt = `
...
Consider the customer's persona when determining satisfaction.`;
    ...
    calculateAverageResponseTime(turns) {
...
//# sourceMappingURL=autonomousConversationService.js.map