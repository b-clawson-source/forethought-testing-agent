...
class AutonomousTestRunner {
    ...
    async runSingleConversation(initialPrompt, personaType, maxTurns, delayBetweenTurns) {
        ...
        while (!conversationComplete && turnCount < maxTurns) {
            ...
            try {
                // Query Forethought
                const forethoughtResponse = await this.forethoughtService.processMessage(currentMessage, sessionId);
                // Generate LLM response based on Forethought's answer
                const llmResponse = await this.llmService.generateTestResponse(currentMessage, forethoughtResponse.response, personaType, result.conversationLog);
    ...
    async generateInitialPrompt(personaType, previousConversation) {
        const context = previousConversation ?
            `Previous conversation ended with: ${previousConversation.conversationLog[previousConversation.conversationLog.length - 1]?.llmResponse || 'no previous context'}` : '';
