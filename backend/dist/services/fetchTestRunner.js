...
const fetchForethoughtService_1 = require("./fetchForethoughtService");
const llmService_1 = require("./llmService");
const fetchPersonas_1 = require("../config/fetchPersonas");
class FetchTestRunner {
    constructor() {
     ...
     */
    async runFetchTestSuite(config) {
        ...
        for (const category of config.testCategories) {
            ...
            const categoryPersonas = (0, fetchPersonas_1.getPersonasByCategory)(category);
            ...
            for (let i = 0; i < config.numberOfConversations; i++) {
                ...
                const result = await this.runFetchConversation(persona, config.maxTurnsPerConversation, config.delayBetweenTurns);
     ...
     */
    async runFetchConversation(persona, maxTurns, delayBetweenTurns) {
        ...
        const initialPrompt = persona.initialPrompts[Math.floor(Math.random() * persona.initialPrompts.length)];
        ...
        while (!conversationComplete && turnCount < maxTurns) {
            ...
            try {
                ...
                // Query Forethought
                const forethoughtResponse = await this.fetchForethoughtService.processMessage(currentMessage, persona.category, userContext, sessionId);
                // Generate LLM response as the customer
                const llmResponse = await this.generateFetchCustomerResponse(currentMessage, forethoughtResponse.response, persona, result.conversationLog);
     ...
     */
    async generateFetchCustomerResponse(userMessage, forethoughtResponse, persona, history) {
...
If not resolved, continue based on your persona's characteristics.`;
     ...
     */
    async generateNextFetchMessage(persona, history) {
...
Stay in character and respond naturally.`;
     ...
     */
    extractCategoryFromConversation(conv) {
    ...
    calculateOverallResolutionRate(report) {
    ...
    calculateAvgTurnsToResolution(report) {
    ...
    calculateIntentAccuracyByCategory(report) {
     ...
     */
    calculateIntentAccuracy(conversationLog) {
...
//# sourceMappingURL=fetchTestRunner.js.map